...
import {
  createRatePlanCategory,
  getAllRatePlanCategory,
  updateRatePlanCategory,
} from "@/services/api";
...
export const TableAdminRatePlan = () => {
  ...
  const onSubmit = async () => {
    try {
      ...
      if (editing) {
        await updateRatePlanCategory(String(editing.id), name);
        message.success("Cập nhật thành công");
      } else {
        await createRatePlanCategory(name);
  ...
  return (
    <div>
      ...
      <Modal
        title={editing ? "Chỉnh sửa gói giá" : "Thêm gói giá "}
        open={open}
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item
            ...
            label="Tên gói giá "
            rules={[
              { required: true, message: "Vui lòng nhập tên gói giá" },
              { min: 1, max: 100 },
            ]}
          >
...
export default TableAdminRatePlan;
